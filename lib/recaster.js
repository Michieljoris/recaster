...
function recast(args) {
    ...
    minify(args)
        ...
        .when(
            function(data) {
                if (encoding !== data.encoding) vow.breek("Failed to encode " + args.srcPath);
                else vow.keep({destData: data.recast, type: data.type, encoding: data.encoding });
            }
            ,function(err) {
                 vow.breek(err);
            });
...
function minify(args) {
    ...
    transpile(args)
        ...
        .when(
            function(data) {
                //pass on the possibly minified js, html or css
                vow.keep({recast: data.recast, type: data.type, err: data.err});
            }
            ,function(err) {
                debug('Couldn\'t minify because we did not receive js, html or css: ' + args.srcPath);
                if (err.recast !== undefined) 
                    //but we can still pass on the data if we have it:
                    vow.keep(err);
                else vow.breek(err);
            });
    return vow.promise;
...
function transpile(args) {
    ...
    readData(args.srcPath, args.srcData, args.type)
        .when(function(data) {
            args.srcData = data.srcData;
            args.type = data.type;
            if (!args.type || !options.transpilerOne[args.type])
                return VOW.broken({ recast: args.srcData, type: args.type, err: "Not transpilable: " + args.type});
        ...
        .when(
            ...
            ,function(err) {
                ...
                if (err.recast !== undefined) 
                    //we've got the data, however it was not transpilable
                    vow.breek({recast: err.recast, type: args.type });
                //data was not read even:
                else vow.breek(err);
            });
    return vow.promise;
}
