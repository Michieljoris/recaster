...
var zlib, less, stylus, jade, marked, Compiler, transformEs6, sjs, tsc, UglifyJS, minifyHtml, csso, cleanCSS;
...
function conditionalRequire() {
    ...
    // //to js
    if (options.transpilerOne.coffeescript)
    ...
    if (options.transpilerOne.sweetjs) sjs = require('sweet.js'); //http://sweetjs.org/
    if (options.transpilerOne.typescript) tsc = require('../node_modules/node-typescript/lib/compiler');//https://npmjs.org/package/node-typescript
...
function isRecastable(ext) {
    if (optionsMap.transpilerOne[ext] ||
        ...
        optionsMap.transpilerTwo[ext]) return true;
    return false;
}
