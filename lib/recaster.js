...
function readData(path, data, type) {
    var vow = VOW.make();
    if (data) vow.keep({ type: type, recast: data.toString() });
    else {
...
function recast(args) {
    ...
    var type = args.type;
    var vow = VOW.make();
    readData(srcFile, srcData, type)
        .when(function(data) {
            srcData = data.recast;
            type = data.type;
            if (!type || !options.transpilerOne[type])
                return VOW.broken('Can\'t package this data: ' + srcFile || '');
        ...
        .when(function(data) {
            ...
            if (options.zip && zip) return transform.zip(data);
