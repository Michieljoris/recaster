...
transform.uglifyJs2 = function(data) {
    ...
    return VOW.kept(extend(data, { recast: recast.code,
                      srcMap: recast.map,
                      minified: true,
                      type: 'js'}));
};
...
transform.denodify = function(data) {
    debug('denodify:', data);
    if (data.isModule)
        return VOW.kept(extend(data, {recast: denodify.wrap(data.path, data.recast)}));
...
//TODO Server configuration could be set up so that the end user can inject his
// own scripts, same as the wsHandlers, strings for built-in scripts such as
// reload and functions for user functions.
...
var scripts = {
    ...
    test: function() { console.log('test script loaded')}
} 

transform.inject = function(data) {
    var vow = VOW.make();
    // debug('inject:', data);
    console.log(data.path,options.inject);
    if (options.inject && options.inject[data.path] && 
        options.inject[data.path].length) {
        var scriptNames = options.inject[data.path];
        var scriptString = '';
        scriptNames.forEach(function(n) {
            if (scripts[n]) scriptString += scripts[n].toString();
            //TODO replace localhost:8080 with actual host and port
            //Plus bb-server has to start up buildMonitor.js with the right host and port as well
        });
        var buffer = '';
...
transform.inject.action = 'inject';
...
function composeFunction(flist) {
    if (flist.length === 0) return function(data) { return VOW.kept(data); };
